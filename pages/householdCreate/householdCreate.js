...
Page({
  ...
  changeSome(e){
    ...
    this.data.household[type] = e.detail
    this.setData({
      household: this.data.household
    })
  },

  //input===== set指定字段数据到household中
  inputSome(e) {
    let type = e.currentTarget.dataset.type
    this.data.household[type] = e.detail.value
    this.setData({
      household: this.data.household
    })
  },
  ...
  gethousehold: function(householdId) {
    return wx.$http.res['_interFace'].getById(householdId).then(res => {
      if (!res.errCode) {
        if (res.data) {
          //  0 =>'0'
  ...
  postInfo: function(e) {
    ...
    if (this.data.type == 'create') {
      wx.$http.post('/api/household', params).then(res => {
        if (!res.errCode) {
          Toast.clear()
          Toast.success('创建成功')
          wx.navigateBack({
      ...
      wx.$http.put('/api/household', params).then(res => {
        if (!res.errCode) {
          Toast.clear()
          Toast.success('修改成功')
          wx.navigateBack({
  ...
  onLoad: function(options) {
    ...
    if (options.type == 'change') {
      ...
      this.gethousehold(options.householdId).then(() => {
      ...
      success: res => {
        wx.uploadFile({
          ...
          success: res => {
            ...
            this.data.household.imageUrl = data.data;
            this.setData({
              household: this.data.household
            })
          }
        ...
        res.tempFilePaths.forEach(item => {
          wx.uploadFile({
            ...
            success: res => {
              ...
              this.data.household.imageUrls.push(data.data);
              this.setData({
                household: this.data.household
              })
            }
...
;
  ...
  // 删除多张图片
  deleteImages(e) {
    this.data.household.imageUrls.splice(e.target.dataset.index, 1)
    this.setData({
      household: this.data.household
    })
  },
